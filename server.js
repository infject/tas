...
function advanceTurn(roomCode) {
  ...
  if (current) {
    ...
    if (current.nextDiscard > 0 && current.hand.length > 0) {
      current.hand.pop();
      current.nextDiscard = 0;
    }
  ...
  do {
    ...
    attempts++;
    if (attempts > room.order.length + 2) break;
  } while (!room.players[room.order[room.turnIndex]] || !room.players[room.order[room.turnIndex]].alive);
...
io.on('connection', socket => {
  ...
  socket.on('playCard', ({ roomCode, cardId, targetId }) => {
    ...
    try { if (card.action) card.action(player, target, room); } 
